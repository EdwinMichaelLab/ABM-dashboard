...
MAX_ROWS=10000000
path = os.path.join('..', 'ABM-simulator', 'SimulationEngine', 'output', 'run-2021-12-29')
...
def load_SEIR(mode):
    if mode=='All cases-Not filtered':
        ...
        # if mode == 'By Age':
        #     return plot_age_old(plotdf)
        # elif mode == 'By Gender':
        #     return plot_gender_old(plotdf)
        # elif mode == 'By Race':
        #     return plot_race_old(plotdf)
        # elif mode == 'By FPL':
        #     return plot_FPL_old(plotdf)
'''
import pymongo
...
SKIP_EVERY_NTH_1=10 # best at 2
SAMPLING_PERCENT_1=0.1 # default 0.25

SKIP_EVERY_NTH_2=2 # best at 2
SAMPLING_PERCENT_2=0.4 # default 0.5

def load_scatter():
    #pdf = pd.read_csv(os.path.join(path, 'scatter.csv'))
    #pdf = pd.read_csv(os.path.join(path, 'scatter.csv'), nrows=MAX_ROWS)
    #pdf = pd.read_csv(os.path.join(path, 'scatter.csv'), skiprows=lambda x: (x != 0) and not x % 2) # even rows
    #pdf = pd.read_csv(os.path.join(path, 'scatter.csv'), skiprows=lambda x: x % 2)
    #pdf = pd.read_csv(os.path.join(path, 'scatter.csv'), chunksize=1000000)
...
def load_heatmap():
    ...
    #pdf = pd.read_csv(os.path.join(path, 'heatmap.csv'), nrows=MAX_ROWS)
    #pdf = pd.read_csv(os.path.join(path, 'heatmap.csv'), skiprows=lambda x: x % 2) # every odd rows
    #pdf = pd.read_csv(os.path.join(path, 'heatmap.csv'), chunksize=1000000)
